function ChatReplay( video_player, panel_id, o_show_title_id, o_subreddit_id )
{
	...
	let timer = null;
	let template = '<dt id="message_%s"><img src="avatars/snoo%d.png" align="left" width="42" height="42"><b><big><a href="http://www.reddit.com/u/%s" target="_blank">%s</a></big></b> &nbsp;<small><font color="gray">%s days ago</font></small></dt><dd>%s</dd>';
	...
	{
		// todo: fix so long lines don't widen table
	...
	{
		let file = fs.OpenTextFile( filespec );
		report = JSON.parse( file.ReadAll( ) );
		file.close( );
